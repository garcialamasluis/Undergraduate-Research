...
male_df <- read.table(file = "./data/raw_data/18192021_complete_data.tsv",
                              header = TRUE,
                              sep = '\t')
...
quasi_binomial_log_model <- glm(cbind(number_of_GFP_kernels, number_of_WT_kernels) ~ # using the ratio between kernels as the output
                            ...
                            data = male_df, # data from  subset, changes depending on expression category
...
Alleles = sort(unique(male_df$allele)) #creating unique allele vector
...
names(estimated_coeffecient) = sort(unique(male_df$allele))
...
analysis_summary = data.frame(allele = Alleles, logit_of_percentage = beta_list, estimated_tranmission_percentage = estimated_coeffecient, lower_confidence_interval = low_int, upper_confidence_interval = up_int, raw_p_value = raw_p, adj_p_value = adjusted_p)
